...
output_file = os.path.join(base_dir, 'Output', 'final_features_v5.csv')
...
for f in files:
    ...
    try:
        ...
        if 'date' not in df_new.columns:
            # Try to find a column that looks like date?
            # The inspection showed 'date' in all of them.
        ...
        # Merge
        # We merge onto df_main. We use left join to keep main's index (dates).
        # If new features have more history, we ignore it? Or should we expand?
        # Usually we want to enrich the existing dataset.
...
print("Missing values in new columns (sample):")
# Check a few new columns
new_cols = [c for c in df_main.columns if c not in pd.read_csv(main_file, index_col=0, nrows=0).columns]
if new_cols:
    print(df_main[new_cols].isna().sum().head())
