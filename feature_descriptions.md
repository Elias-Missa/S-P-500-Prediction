...
- **Description**: Trailing percentage returns over 1, 3, 6, and 12 months.
- **Formula**: $(Price_t - Price_{t-n}) / Price_{t-n}$ where $n \approx 21, 63, 126, 252$.
- **Logic**: Captures momentum over various time horizons.

### `Slope_50`, `Slope_100`
- **Description**: Slope of the linear regression line of prices over 50 and 100 days.
- **Formula**: Slope $m$ from $Price = m \times Time + c$.
- **Logic**: Measures the direction and steepness of the trend. Positive slope = uptrend, Negative = downtrend.
...
### `Sectors_Above_50MA`
- **Description**: Percentage of S&P 500 Sector ETFs trading above their 50-day Moving Average.
- **Proxy**: Calculated using the 9 Select Sector SPDRs (XLK, XLF, XLV, XLY, XLP, XLE, XLI, XLB, XLU).
- **Formula**: $(\sum_{i=1}^{9} \mathbb{I}(Price_i > SMA_{50,i})) / 9 \times 100$
- **Logic**: A market breadth indicator. Measures participation across different sectors of the economy. Values range from 0 to 100 in steps of ~11.1%.
